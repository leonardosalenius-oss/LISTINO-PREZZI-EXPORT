...
var PRODOTTI_HEADERS = ["id","categoria","nome","formato","prezzo_base","unita_base","peso_unitario_kg","disponibile","traduzioni_json","ordine","immagine_url","confezione_kg"];
...
// Migrazione additiva: aggiunge la colonna confezione_kg senza toccare i dati
// esistenti. Sicura da eseguire piu' volte.
function aggiungiCampoConfezione() {
  var sheet = getSheet_("Prodotti");
  var lastCol = sheet.getLastColumn();
  var headers = sheet.getRange(1, 1, 1, lastCol).getValues()[0];
  if (headers.indexOf("confezione_kg") === -1) {
    sheet.getRange(1, lastCol + 1).setValue("confezione_kg");
    Logger.log("Colonna 'confezione_kg' aggiunta in posizione " + (lastCol + 1) + ". I prezzi esistenti NON sono stati toccati.");
  } else {
    Logger.log("La colonna 'confezione_kg' esiste gia', nessuna modifica necessaria.");
  }
}
